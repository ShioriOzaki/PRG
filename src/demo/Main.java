...
public class Main {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			// 人間グループから1人選択
			Human Fighthuman = choiceHuman(humans);
			
			// モンスターグループから1人選択
			Monster FightMonster = choiceMonster(monsters);
            
			// 選ばれた人間が、選ばれたモンスターを攻撃
			Fighthuman.attack(FightMonster);
			
			// モンスターのHPが0以下になれば、モンスターは倒れ、そのモンスターをモンスターグループから削除
			int FightMonsterhp = FightMonster.getHp();
			if (FightMonsterhp <= 0) {
				monsters.remove(FightMonster);
				System.out.printf("\n★%sは倒れた",FightMonster.getName());
			}
			...
			// 人間グループから1人選択
			Fighthuman = choiceHuman(humans);
			
			// モンスターグループから1人選択
			FightMonster = choiceMonster(monsters);
			
			// 選ばれたモンスターが、選ばれた人間を攻撃
			FightMonster.attack(Fighthuman);

			// 人間のHPが0以下になれば、人間は倒れ、その人間をモンスターグループから削除
			int FightHumanhp = Fighthuman.getHp();
			if (FightHumanhp <= 0) {
				humans.remove(Fighthuman);
				System.out.printf("\n★%sは倒れた\n",Fighthuman.getName());
			}
